...
%           filt = max(0, min(1, 2 - abs(fRange) ./ (-fRange(1+floor(end/2)) * fraction))); % linear roll-off
    ...
    if ~all(S(:) == 0)
      ...
      LpI_inv = @(x) to_vec(convolveDyadicG(-1i*imag(alpha) .* to_array(x)));
      G = @(x) to_vec(chiMul(to_array(x)) .* (1i/imag(alpha)) );
      Ek02_sr = splitrichardson(LpI_inv, G, to_vec(S ./ (-1i*imag(alpha)) ), [], [], to_vec(Ek02)*0, progressFunction);
      Ek02_sr = to_array(Ek02_sr);
      resultE_sr = (k0^-2) *  reshape(Ek02_sr, [output_shape 1]);
      ...
      % Compare
      %
      logMessage('Relative difference in result: %d.\n Time splitrichardson/specific: %0.1f/%0.1f = %0.3f', [norm(resultE(:)-resultE_sr(:)) / norm(resultE(:)), time_sr, time_specific, time_sr/time_specific]);
    else
