function [resultE, rmsError, itIdx] = solveMacroscopicMaxwell(ranges, k0, epsilon, xi, zeta, mu, sourceDistribution, progressFunction, E, lowPassBands)
    ...
    compare_with_specific_algorithm = false;
...
%           filt = max(0, min(1, 2 - abs(fRange) ./ (-fRange(1+floor(end/2)) * fraction))); % linear roll-off
    ...
    if ~all(S(:) == 0)
      ...
      Ek02 = lowPass(Ek02, lowPassIteration);  % Only applied when requested
      ...
      tic;
      % Define helper functions to vectorize and de-vectorize field distributions
      to_vec = @(E) E(:);
      to_array = @(x) reshape(x, [round(numel(x)/prod(data_shape)), 1, data_shape]);
      % Define inv(H+G) and G as functions
      HpG_inv = @(x) to_vec(convolveDyadicG(-1i*imag(alpha) .* to_array(x)));
      G = @(x) to_vec(chiMul(to_array(x)) .* (1i/imag(alpha)) );
      % Use split-Richarson to solve the problem
      Ek02 = splitrichardson(HpG_inv, G, to_vec(S ./ (-1i*imag(alpha)) ), [], [], to_vec(Ek02), progressFunction);
      % Convert back to field-shape and undo the normalizing scaling.
      resultE = (k0^-2) *  reshape(Ek02, [output_shape 1]);
      
      if compare_with_specific_algorithm
          time_sr = toc;

          %
          % Also run specific algorithm for comparison
          %
          Ek02 = Ek02_init;
          tic;
          ...
          while cont
              ...
              correction = chiMul(Ek02);  % up to 4 FTs
              correction = correction + S;
              correction = convolveDyadicG(correction);  % 2 FTs
              correction = correction - Ek02;
              correction = gammaMul(correction);  % up to 4 FTs
              % % The above as a one-liner:
              % correction = gammaMul(convolveDyadicG(chiMul(Ek02) + S) - Ek02);

              correction = lowPass(correction, lowPassIteration);  % Only applied when requested

              % Check convergence criterion
              currentUpdateNorm = norm(correction(:));
              relnorm = currentUpdateNorm / previousUpdateNorm;
              if relnorm < 1
                ...
                logMessage('Convergence criterion not fullfilled!\nEigenvalue of M is at least as large as %d!', relnorm);
              ...
              if narginUpdateFunctor >= 2
                  ...
                  residue = norm(correction(:)) / norm(Ek02(:));
                  argIn{2} = residue;
              end
          ...
          resultE_specific = (k0^-2) * reshape(Ek02, [output_shape 1]);
          ...
          % Compare
          %
          logMessage('Relative difference in result: %d.\n Time splitrichardson/specific: %0.1f/%0.1f = %0.3f', [norm(resultE_specific(:)-resultE(:)) / norm(resultE(:)), time_sr, time_specific, time_sr/time_specific]);
      end
    else
...
function [alpha, beta] = calcBiases(epsilon, xi, zeta, mu, samplePitch, k0, maxKernelRadiusInRad, maxKernelResidue)
  ...
  if isMagnetic
    ...
    alpha_beta_vec0 = [0 1];
    [alpha_beta_vec, ~] = fminsearch(targetFunctionVec, alpha_beta_vec0, optimset('TolX', 1e-4, 'TolFun', 1e-4, 'MaxIter',100, 'MaxFunEvals', 2*100, 'Display', 'none'));
